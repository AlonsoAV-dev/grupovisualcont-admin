...
// Configuración para excluir del static export
...
export default function NoticiaEditor() {
  ...
  const loadData = async () => {
    try {
      const [autoresRes, categoriasRes, keywordsRes] = await Promise.all([
        ...
        authFetch('/api/keywords/all'), // Usar endpoint SIN paginación
      ]);
  ...
  const loadNoticia = async () => {
    try {
      ...
      if (data.noticia) {
        ...
        // IMPORTANTE: Agregar las keywords de la noticia a la lista local para visualizarlas
        if (data.noticia.keywords && data.noticia.keywords.length > 0) {
          setKeywords(prev => {
            const existingIds = prev.map(k => k.id_keyword);
            const newKeywords = data.noticia.keywords.filter(k => !existingIds.includes(k.id_keyword));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        let errorMessage = data.error || 'Error al guardar noticia';
  ...
  // Agregar keyword sugerida (SIEMPRE buscar primero en BD)
  const agregarKeywordSugerida = async (sugerencia) => {
    // SIEMPRE buscar en la BD primero para asegurar que existe o no
    let existingKeyword = null;

    try {
      const searchRes = await authFetch(`/api/keywords/search?q=${encodeURIComponent(sugerencia.trim())}`);
      const searchData = await searchRes.json();
    ...
    // Si existe en la BD, usar su ID real
    if (existingKeyword) {
      // Agregar al formData si no está ya seleccionada
    ...
    // Si NO existe en la BD, crear una keyword TEMPORAL (se creará en BD al guardar)
    const tempKeyword = {
      id_keyword: -(Date.now()), // ID temporal único negativo
      nombre: sugerencia,
    };
                ...
                <div className="flex flex-wrap gap-2 mb-3 p-3 bg-gray-50 dark:bg-gray-900 rounded-md">
                  {formData.keywords.map((keywordId) => {
                    ...
                    return (
                      <span
                        key={keywordId}
                        className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm ${isTemporary
