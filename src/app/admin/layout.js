...
export default function AdminLayout({ children }) {
  ...
  useEffect(() => {
    // Solo verificar auth si NO estamos en login
    if (!isLoginPage) {
      checkAuth();
    } else {
      setLoading(false);
  ...
  // Si estamos en la página de login, solo mostrar el children
  if (isLoginPage) {
    return <>{children}</>;
  }
  ...
  if (user.rol === 'admin') {
    ...
    menuItems.push({ href: '/admin/categorias', icon: faLayerGroup, label: 'Categorías' });
  }
